...
JNIEXPORT void JNICALL Java_com_seeta_sdk_QualityOfPoseEx_construct__Lcom_seeta_sdk_SeetaModelSetting_2
  ...
  (JNIEnv *env, jobject self, jstring model, jstring device, jint id)
  {
	...
	jint device_id = 0;
	if(native_device.compare("CPU") == 0){
		device_id = 1;
	}else if(native_device.compare("GPU") == 0){
		device_id = 2;
	}
	seeta::ModelSetting model_setting(native_model, (seeta::ModelSetting::Device)device_id, id);
	//seeta::ModelSetting_v2 model_setting(native_model, (seeta::ModelSetting::Device)device_id, id);
